...
$i$ 回目のイテレーションでの回転因子を $Omega_i$ とおくと、$Omega_(i+1) = Omega_i omega_n^("reverse"(i+1) - "reverse(i)")$ が成り立ちます。
ここで、$"reverse()"$ は $d = log_2 n$ ビット整数としてのビット反転操作です。
$i = ****1011_((2))$ とすると $i+1 = ****1100_((2))$ なので、$ "reverse"(i+1) - "reverse"(i) & = 0011****_((2)) - 1101****_((2)) \
                              & = 2^(d-3) - 2^(d-2) - 2^(d-1) \
        therefore Omega_(i+1) & = Omega_i omega_2^1 omega_4^(-1) omega_8^(-1) $とかけます。
...
一方で、周波数間引きアルゴリズムでは部分問題の入力を計算していくと自動的に数論変換が終了しています。
