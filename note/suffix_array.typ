...
接尾辞の長さが高々 $|S|$ だからです。
LMS型接尾辞を比較することを考えると、２つのLMS型文字に挟まれてできる部分文字列ごとに比較すればよいことが分かります。
...
#lemma[
  接尾辞S[SA[i]$plus.minus$1..]の少なくとも一方は接尾辞S[SA[i]..]とのLCPが最大の接尾辞である。
]
#corollary[
...
この性質を使ってLCPを高速に計算するためには接尾辞配列ではなく文字列Sを正順で走査するとよい。
  ...
  fn lcp_array(text: &[usize], sa: &[usize]) -> Vec<usize> {
    ...
    for i in 0..sa.len() {
      if lcp[i] > 0 {
        ...
        // lcp[i] はもう不要なので、LCPの長さを書き込む。メモリの節約。
        lcp[i] = l;
...
#proof[
  ...
  なぜなら、$l$ は高々 $|S|$ であり、高々 $|S|$ 回デクリメントされるので、while ループが高々 $2|S|$ 回実行されるから。
]

\

LCP配列を接尾辞の昇順に並べ替えるためには、SA[i]とLCP[SA[i]]をスワップすればよい。
このとき、SAは接尾辞の昇順のLCP配列となり、LCPは接尾辞配列の逆関数になっている。

= 接尾辞木

接尾辞木とは文字列Sのすべての接尾辞からなるTrie木である。
これは $O(|S|^2)$ のメモリを使用するが、パトリシア木のように $Theta(|S|)$ 圧縮できる。
接尾辞配列とLCP配列があるとき、接尾辞木での深さ優先探索をシミュレートできることが知られている。
この手法はメモリアクセスが連続的であり、辺を明示的に管理しなくてよい点で優れている。
本稿では接尾辞木は圧縮されているものとする。

#lemma[
  （圧縮された）接尾辞木のノード数は高々 $2|S|-1$ である。
] <lemma:suffix-tree-size>
#proof[
  接尾辞を昇順に追加することを考える。
  最初を除いて、接尾辞を１つ追加する度にノードが１つか２つ増える。
]
...
#inline-note[DFSのアルゴリズム。帰りがけは簡単だが、行きがけは難しそう]
...
ユニークな区切り文字 $"#"$ と番兵 $"0"$ について、新しい文字列 $"S" + "#" + "T" + "0"$ を構築し、接尾辞配列とLCP配列を計算する。
接尾辞の長さと区切り文字がユニークであることから、Sの接尾辞とTの接尾辞が区別できる。
接尾辞配列で隣接するSの接尾辞とTの接尾辞のLCPが最大のものが解である。
