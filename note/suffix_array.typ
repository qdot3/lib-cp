...
#proof[
  カウントソートにより、S型接尾辞をSAの正しい場所に格納できる。
  SAを前から走査する。
  SA[i]が添え字であり、S[$"SA[i]"-1$]がL型のとき、対応するバケットの左端に$"SA[i]"-1$を書き込む。
  このとき、バケットの左端を指すカウンターをインクリメントする。
  L型の定義より、アルゴリズムは正しい。
  計算量は $Theta(|S|)$ である。
]
...
#definition[
  LMS型文字から次のLMS型文字までの部分列をLMS型部分文字列という。
]

LMS型接尾辞をソートするためには、LMS型部分文字列ごとに比較すればよい。
すべてのLMS型部分文字列を順序を保ったまま新しい文字で置き換えることができれば、サイズが高々 $abs(S)/2$ の問題に帰着できる。

#theorem[
  LMS型部分文字列を線形時間でソートできる。
]

#proof[
  LMS型文字はカウントソートで線形時間でソートできる。
  これに誘導ソートを適用すると、頭文字を除いて最初のLMS型文字までソートできる。
  つまり、LMS型部分文字列を線形時間でソートできる。
]

#corollary[
  座標圧縮により部分問題を線形時間で構築できる。
]
...
#proof[
  計算量は $T(n) <= T(n/2) + Theta(n) <= Theta(n)$ である。
]
